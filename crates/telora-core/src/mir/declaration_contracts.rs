...
impl Mir {
    fn contract_hole(&self, annotation: HirId) -> Option<HirId> {
        ...
        while let Some(node) = pending.pop() {
            ...
            if matches!(syntax.kind, HirKind::InferredTypeArgument) {
                return Some(node);
            }
    ...
    fn declaration_contract_image(&self) -> Vec<DeclarationContract> {
        ...
        for (index, definition) in self.symbols.iter().enumerate() {
            ...
            if definition.declarations.iter().all(|node| {
                matches!(
        ...
        for &export in self.exports.iter().flatten() {
            let ResolveState::Bound(symbol) = self.symbols[export.index()].resolution else {
                continue;
            };
            if let Some(exports) = declarations.get_mut(&symbol) {
                exports.push(export);
            }
        }
        declarations
            .into_iter()
            .filter_map(|(symbol, exports)| {
                    ...
                    Some(annotation) => match self.contract_hole(annotation) {
                            ...
                            .and_then(|slot| self.ty_slots.get(slot.index()))
                        {
                            Some(TypeState::Known(ty))
                                if self.declaration_contract_ready.get(symbol.index())
                                    == Some(&true) =>
                            {
        ...
        for contract in &self.declaration_contracts {
            let diagnostic = match contract.state {
                ...
                DeclarationContractState::Incomplete(hole) => Diagnostic::error(
                    "top-level type contract cannot contain an inference hole",
                    self.hir[hole.index()].location,
                ),
