...
impl Solver<'_> {
    pub(super) fn annotation_sources(mir: &Mir) -> Vec<Option<HirId>> {
        let symbols = mir.symbols.iter().map(|symbol| symbol.declarations.iter().find_map(|node|
            mir.hir[node.index()].children.iter().find(|edge| edge.role == Role::Annotation)
                .map(|edge| edge.node))).collect::<Vec<_>>();
        let mut sources = vec![None; mir.hir.len()];
        for (index, symbol) in mir.symbols.iter().enumerate() {
            for node in &symbol.declarations { sources[node.index()] = symbols[index]; }
        }
    ...
    pub(super) fn contract_source(&self, mut node: HirId) -> Option<HirId> {
        loop {
            if let Some(source) = self.annotation_sources[node.index()] { return Some(source); }
            if let Some(annotation) = self.child(node, Role::Annotation) { return Some(annotation); }
    ...
    pub(super) fn fit_source(&mut self, node: HirId, actual: TypeSlotId) -> Option<HirId> {
        let contract = self.constraint_contract.or_else(|| self.contract_source(node));
    ...
    pub(super) fn failed_contract_sources(&self) -> Vec<HirId> {
        if let Some(contract) = self.constraint_contract { return vec![contract]; }
        self.constraint_origin.map(|node| self.contract_uses[node.index()].clone()).unwrap_or_default()
    }
}
