...
impl Solver<'_> {
    pub(super) fn solve_contracts(&mut self) -> Vec<bool> {
        ...
        for (index, &generate) in generated.iter().enumerate() {
            if generate {
                self.generate(HirId(index as u32));
            }
        }
        ...
        loop {
            ...
            for task in pending {
                if let Some(task) = self.solve_task(task) {
        ...
        self.mir.declaration_contract_ready = self
            .mir
            .symbol_types
            .iter()
            .enumerate()
            .map(|(index, slot)| {
                self.closed_contract(*slot)
                    && self.mir.symbol_generics[index].iter().all(|parameter| {
                        self.mir.symbols[parameter.index()]
                            .declarations
                            .iter()
                            .all(|node| {
                                self.children(*node, Role::Bound)
                                    .iter()
                                    .all(|bound| self.closed_contract(bound.ty()))
                            })
                    })
            })
            .collect();
    ...
    fn protect_contracts(&mut self) {
        let mut pending = self
            ...
            .iter()
            .enumerate()
            .filter_map(|(index, slot)| self.mir.declaration_contract_ready[index].then_some(*slot))
            .collect::<Vec<_>>();
        while let Some(slot) = pending.pop() {
            let root = self.root(slot);
            if self.contract_slots[root.index()] {
                continue;
            }
            let Some(term) = self.term(root).cloned() else {
                continue;
            };
            ...
            {
                pending.extend(
                    self.mir.type_definitions[index]
                        .members
                        .iter()
                        .filter_map(|member| member.payload),
                );
            }
        }
    }
    ...
    fn closed_contract(&self, slot: TypeSlotId) -> bool {
        ...
        while let Some(slot) = pending.pop() {
            ...
            let Some(term) = self.term(root) else {
                return false;
            };
