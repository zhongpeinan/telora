...
impl Solver<'_> {
    pub(super) fn resolve_constructor_patterns(&mut self) {
        for index in 0..self.mir.hir.len() {
            ...
            ) || self.term(node.ty()).is_none()
            {
                continue;
            }
            let mut callee = if matches!(self.mir.hir[index].kind, HirKind::PatternName(_)) {
                if self.mir.hir_symbols[index].is_some_and(|symbol| {
                ...
                self.child(node, Role::Callee).unwrap()
            };
            ...
            while seen.insert(callee) {
                match self.mir.member_selections[callee.index()] {
                    ...
                    Some(
                        selection @ (MemberSelection::EnumVariant { .. }
                        | MemberSelection::Boolean(_)),
                    ) => {
                        if value_alias && matches!(selection, MemberSelection::EnumVariant { .. }) {
                            break;
                        }
                ...
                let Some(next) = self.child(callee, role) else {
                    break;
                };
                callee = next;
            }
            if !selected {
                self.conflict(
                    node.ty(),
                    node.ty(),
                    Some(self.mir.hir[index].location),
    ...
    pub(super) fn prepare_type_uses(&mut self) {
                ...
                if matches!(edge.role, Role::Annotation | Role::Bound)
                    || matches!(
                        ...
                        (HirKind::TypeMetadata | HirKind::TypeSyntax, Role::Operand)
                            | (
                                HirKind::TypeApply | HirKind::TypeOperation(_),
                                Role::Argument
                            )
                            | (HirKind::TypeAscription, Role::Target)
                            | (
                                HirKind::Binding {
                                    kind: BindingKind::Type | BindingKind::Trait,
                                    ..
                                },
                                Role::Value
                            )
        ...
        while let Some(node) = pending.pop() {
            if std::mem::replace(&mut self.type_uses[node.index()], true) {
                continue;
            }
            pending.extend(
                self.mir.hir[node.index()]
                    .children
                    .iter()
                    .filter(|edge| edge.role != Role::Decorator)
                    .map(|edge| edge.node),
            );
        }
    }

    pub(super) fn type_facet(
        &mut self,
        node: HirId,
        source: TypeSlotId,
        finish: bool,
        ...
        if matches!(
            self.mir.ty_slots[self.root(source).index()],
        ...
        let Some(term) = self.term(source).cloned() else {
            return Some(Task::TypeFacet { node, source });
        };
        ...
        let Some(raw) = self.term(owner).cloned() else {
            return Some(Task::TypeFacet { node, source });
        };
        ...
        if !finish && self.term(node.ty()).is_none() {
            return Some(Task::TypeFacet { node, source });
        }
        let TypeConstructor::Nominal(symbol) = raw.constructor else {
            unreachable!()
        };
    ...
    pub(super) fn finish_type_facets(&mut self) -> bool {
        ...
        for task in pending {
            if let Task::TypeFacet { node, source } = task {
                if let Some(task) = self.type_facet(node, source, true) {
