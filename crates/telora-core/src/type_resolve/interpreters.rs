...
impl Solver<'_> {
    ...
    pub(super) fn prepare_interpreter(&mut self, node: HirId) {
        let binding = self
            ...
            .find(|(_, hir)| {
                matches!(
                    ...
                    .children
                    .iter()
                    .any(|edge| edge.role == Role::Value && edge.node == node)
            })
        ...
        {
            self.tasks.push(Task::Interpreter {
                node,
                parameters: self.mir.symbol_generics[symbol.index()].clone(),
            });
        } else {
            self.interpreter_error(
                node,
                "interpreter requires a directly annotated generic def",
            );
        }
    }
    ...
    fn interpreter_contains_parameter(
        &self,
        slot: TypeSlotId,
        parameters: &[SymbolId],
    ...
    pub(super) fn interpreter(&mut self, node: HirId, parameters: Vec<SymbolId>) -> Option<Task> {
        let pending = || {
            Some(Task::Interpreter {
                node,
                parameters: parameters.clone(),
            })
        };
        if matches!(
            self.mir.ty_slots[self.root(node.ty()).index()],
            TypeState::Conflicted(_)
        ) {
            return None;
        }
        let Some(outer) = self.term(node.ty()).cloned() else {
            return pending();
        };
        if outer.constructor != TypeConstructor::Function || outer.arguments.is_empty() {
            self.interpreter_error(
                node,
                "interpreter requires a witness function returning a function",
            );
            return None;
        }
        let mut witnesses = Vec::new();
        for (index, &slot) in outer.arguments[..outer.arguments.len() - 1]
            .iter()
            .enumerate()
        {
            let Some(witness) = self.term(slot) else {
                return pending();
            };
            ...
            let Some(subject) = self.term(witness.arguments[0]) else {
                return pending();
            };
            let TypeConstructor::Parameter(parameter) = subject.constructor else {
                self.interpreter_error(
                    node,
                    "interpreter witness must name a quantified type parameter",
                );
                return None;
            };
        ...
        if witnesses.len() != parameters.len() {
            let missing = parameters
                .iter()
                .find(|&&parameter| !witnesses.iter().any(|&(p, _)| p == parameter))
                .unwrap();
        ...
        let Some(inner) = self.term(*outer.arguments.last().unwrap()).cloned() else {
            return pending();
        };
        ...
        for (index, &slot) in inner.arguments[..inner.arguments.len() - 1]
            .iter()
            .enumerate()
        {
            let Some(term) = self.term(slot) else {
                return pending();
            };
            if let TypeConstructor::Parameter(parameter) = term.constructor
        ...
        let mut erased = erased
            .into_iter()
            .map(|slot| slot.unwrap_or_else(|| self.structure(TypeConstructor::Dyn, vec![])))
            .collect::<Vec<_>>();
        erased.push(result);
