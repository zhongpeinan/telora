...
impl Solver<'_> {
    pub(super) fn record_spread(&mut self, node: HirId) -> Option<Task> {
        ...
        for field in self.children(node, Role::Field) {
            ...
            if let Some(name) = self.child(field, Role::Name) {
                ...
                let Some(term) = self.term(value.ty()).cloned() else {
                    if matches!(
                        self.mir.ty_slots[self.root(value.ty()).index()],
                ...
                if matches!(term.constructor, TypeConstructor::Record(_)) {
                    if self
                        .term(node.ty())
                        .is_some_and(|target| target.constructor == TypeConstructor::Dict)
                    {
                        self.equal(
                            value.ty(),
                            node.ty(),
                            Some(self.mir.hir[value.index()].location),
                        );
                    }
                    return Some(Task::RecordSpread { node });
                }
                if term.constructor == TypeConstructor::Dict {
                    if named {
                        self.projection_error(
                            node,
                            "cannot mix Dict and named struct spreads".into(),
                        );
                        return None;
                    }
                ...
                let Some((TypeOperation::Struct, members)) = members else {
                    self.projection_error(
                        node,
                        "record spread requires a named struct operand".into(),
                    );
                    return None;
                };
                named = true;
                for (name, payload) in members {
                    fields.insert(name, payload.unwrap());
                }
            }
        }
        if let Some(element) = dictionary {
            for actual in items {
                if actual.index() < self.mir.hir.len() {
                    self.fit(HirId(actual.0), element, actual);
                } else {
                    self.equal(element, actual, Some(self.mir.hir[node.index()].location));
                }
            }
        ...
        self.assign(
            node,
            TypeConstructor::Record(fields.keys().cloned().collect()),
            fields.into_values().collect(),
        );
        None
    }

    pub(super) fn struct_update(
        &mut self,
        node: HirId,
        left: TypeSlotId,
        right: TypeSlotId,
    ) -> Option<Task> {
        let (Some(target), Some(patch)) = (self.term(left).cloned(), self.term(right).cloned())
        else {
        ...
        let fields = match patch.constructor {
            TypeConstructor::Record(names) => {
                names.into_iter().zip(patch.arguments).collect::<Vec<_>>()
            }
            TypeConstructor::Nominal(symbol) => {
                let Some((TypeOperation::Struct, members)) =
                    self.nominal_members(symbol, &patch.arguments)
                else {
                    self.projection_error(
                        node,
                        "struct update requires a named struct operand".into(),
                    );
                    return None;
                };
                members
                    .into_iter()
                    .map(|(name, payload)| (name, payload.unwrap()))
                    .collect()
            }
            _ => {
                self.projection_error(node, "struct update requires a named struct operand".into());
    ...
    pub(super) fn field_projection(&mut self, node: HirId, receiver: TypeSlotId) -> Option<Task> {
        let Some(term) = self.term(receiver).cloned() else {
            if matches!(
                self.mir.ty_slots[self.root(receiver).index()],
                TypeState::Conflicted(_)
            ) {
        ...
        let Some((TypeOperation::Struct, members)) = members else {
            self.projection_error(
                node,
                "field projection requires a named struct source".into(),
            );
            return None;
        };
        let mut fields = BTreeMap::new();
        for (source, target) in self
            .children(node, Role::Name)
            .into_iter()
            .zip(self.children(node, Role::Target))
        {
            let HirKind::Name(source) = &self.mir.hir[source.index()].kind else {
                unreachable!()
            };
            let HirKind::Name(target) = &self.mir.hir[target.index()].kind else {
                unreachable!()
            };
    ...
    pub(super) fn validate_field_projections(&mut self) {
        let contributions = self
            ...
            .iter()
            .enumerate()
            .filter(|(_, node)| matches!(node.kind, HirKind::Binary(BinaryOperator::StructUpdate)))
            .filter_map(|(index, _)| self.child(HirId(index as u32), Role::Right))
        ...
        for index in 0..self.mir.hir.len() {
                    ...
                    .any(|&field| self.child(field, Role::Name).is_none())
                {
                    for field in self.children(node, Role::Field) {
                        if self.child(field, Role::Name).is_none() {
                            let value = self.child(field, Role::Value).unwrap();
                            if self.term(value.ty()).is_some_and(|term| {
                                matches!(term.constructor, TypeConstructor::Record(_))
                            }) {
                                self.projection_error(
                                    node,
                                    "record spread requires a named struct operand".into(),
                    ...
                    if !contributions.contains(&node)
                        && self.term(node.ty()).is_some_and(|term| {
                            matches!(term.constructor, TypeConstructor::Record(_))
                        })
                    {
                        self.projection_error(
                            node,
                            "record spread requires a named struct target context".into(),
                        );
                    }
                }
            }
            ...
            if !matches!(self.mir.hir[index].kind, HirKind::FieldProjection) {
                continue;
            }
            let node = HirId(index as u32);
            ...
            let Some(term) = self.term(node.ty()) else {
                continue;
            };
            if !matches!(term.constructor, TypeConstructor::Nominal(_)) {
                self.projection_error(
                    node,
                    "field projection requires a named struct target context".into(),
                );
            }
        }
    }
