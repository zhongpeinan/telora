...
impl Solver<'_> {
    ...
    fn empty_option_value(&self, mut node: HirId) -> bool {
        ...
        loop {
            ...
            if let Some(MemberSelection::EnumVariant { index: 0 }) =
                self.mir.member_selections[node.index()]
            {
                return self
                    .term(node.ty())
                    .is_some_and(|term| term.constructor == TypeConstructor::Option);
            }
            ...
            let Some(&declaration) = self.mir.symbols[symbol.index()].declarations.first() else {
                return false;
            };
    ...
    pub(super) fn finish_empty_options(&mut self) -> bool {
        ...
        for index in 0..self.mir.hir.len() {
            ...
            if term.constructor != TypeConstructor::Option {
                continue;
            }
            let argument = self.root(term.arguments[0]);
            if self.mir.ty_slots[argument.index()] != TypeState::Unknown
                || !self.empty_option_value(node)
            {
                continue;
            }
