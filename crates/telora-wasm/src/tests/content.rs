...
fn source() -> String {
    std::fs::read_to_string(concat!(env!("CARGO_MANIFEST_DIR"),
        "/tests/fixtures/content-slices.telora")).unwrap()
}
...
fn shared_content_moves_once_and_short_views_release_their_parent() {
    ...
    let entry = Value { pointer: session.entry().unwrap(), ty: session.manifest.entry_type };
    ...
    assert_eq!(content_len(&session), baseline + text.len() as u32,
    ...
    for size in [1000, 10000, 100000] {
        session.input_value(string, &"x".repeat(size).into()).unwrap();
    }
    ...
    for _ in 0..3 {
        ...
        assert_eq!(content_len(&session), baseline + 38,
            "copy the live envelope once, trim head/tail, retain the interior gap");
        let output = session.output();
        let (base, _) = output.payload(ARRAYS, output.word(result.pointer as u64 + DATA).unwrap()).unwrap();
        let raw = output.word(base + DATA + 8).unwrap();
        assert_eq!(raw, output.word(base + 2 * u64::from(STRING_BYTES) + DATA + 8).unwrap());
        assert_eq!(output.bytes(base + u64::from(STRING_BYTES) + DATA + 15, 1).unwrap(), [4]);
    }
...
fn bytes_use_the_same_fifteen_byte_inline_boundary_as_strings() {
    ...
    let output = session.output();
    let (base, _) = output.payload(RECORDS, output.word(result.pointer as u64 + DATA).unwrap()).unwrap();
    for (index, expected) in [b"".as_slice(), b"123456789012345", b"1234567890123456"].into_iter().enumerate() {
        let pointer = base + index as u64 * u64::from(STRING_BYTES);
        assert_eq!(output.content_bytes(pointer).unwrap(), expected);
        assert_eq!(output.bytes(pointer + DATA + 15, 1).unwrap(), [expected.len() as u8]);
    }
    assert_eq!(content_len(&session), baseline + 16);
    let result = session.collect_work(&[result]).unwrap().0[0];
    assert!(session.output().debug_repr(result.pointer as u64).is_ok());
}
