...
fn host_buffers_share_allocator_layout_and_preserve_reallocated_bytes() {
    ...
    for align in [1, 2, 8, 16, 64] {
        ...
        for _ in 0..16 {
            ...
            let grown = realloc.call(&mut session.store, (pointer, 262144, 524288, align)).unwrap();
            free.call(&mut session.store, (grown, 524288, align)).unwrap();
            let size = session.memory.data_size(&session.store);
            if let Some(expected) = plateau { assert_eq!(size, expected); }
            plateau = Some(size);
        }
        session.initialize().unwrap();
        let host_buffer = alloc.call(&mut session.store, (32, align)).unwrap();
        session.memory.write(&mut session.store, host_buffer as usize, b"host-owned").unwrap();
        session.collect_work(&[]).unwrap();
        assert_eq!(&session.memory.data(&session.store)[host_buffer as usize..host_buffer as usize + 10],
            b"host-owned");
        free.call(&mut session.store, (host_buffer, 32, align)).unwrap();
    }
