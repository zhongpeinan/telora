...
impl Emitter<'_> {
    ...
    pub fn service_reply_native(&mut self) -> Result<u32, String> {
        ...
        if args.len() != 4
            ...
            || self.mir.types[args[2].index()].constructor != T::Array
            || args[3] != args[0]
        {
    ...
    fn reply_value(&mut self, ty: TypeId, value: u32, writer: u32) -> Result<(), String> {
        match self.mir.types[ty.index()].constructor {
            T::String => {
                self.json_write(2, writer, value);
            }
            T::Int => {
                self.json_write(3, writer, value);
            }
            T::Bool => {
            ...
            T::Enum(_) | T::Nominal(_) if !self.plan.layouts[ty.index()].variants.is_empty() => {
                let variants = self.plan.layouts[ty.index()]
                    .variants
                    .iter()
                ...
                for (index, (name, payload)) in variants.iter().enumerate() {
                    if payload.is_some() {
                        return Err("Wasm: diagnostic enum has payload".into());
                    }
            ...
            T::Record(_) | T::Nominal(_) => {
                let members = self.plan.layouts[ty.index()]
                    .object
                    .as_ref()
                    .ok_or("Wasm: diagnostic record has no layout")?
                    .members
                    .iter()
                    .map(|m| (m.name.clone(), m.offset, m.type_id))
                    .collect::<Vec<_>>();
                ...
                for (index, (name, offset, field_ty)) in members.iter().enumerate() {
                    ...
                    self.extend([
                        I::LocalGet(base),
                        I::I32Const(offset.ok_or("Wasm: missing diagnostic field offset")? as i32),
                        I::I32Add,
                        I::LocalSet(field),
                    ]);
                    let field_ty = self.plan.layouts
                        [field_ty.ok_or("Wasm: missing diagnostic field type")?]
                    .id();
            ...
            ref other => {
                return Err(format!(
                    "Wasm: unsupported service diagnostic field {ty:?}: {other:?}"
                ));
            }
        }
        Ok(())
