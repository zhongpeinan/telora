...
pub(super) fn validate(mir: &Mir, symbol: SymbolId, context: bool) -> Result<(), String> {
    ...
    let message = if context { "eval-with export: expected Eval (std/entry.Eval)" }
        else { "eval export: expected Value (std/value.Value)" };
    let (module, name) = if context { ("std/entry", "Eval") } else { ("std/value", "Value") };
